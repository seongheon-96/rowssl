...
from model.model_ours3 import DINOHead, SupConLoss_2,info_nce_logits, info_nce_logits_tail, SupConLoss,DistillLoss4,DistillLoss4_var, DistillLoss,DistillLoss4_weight, ContrastiveLearningViewGenerator, get_params_groups
...
def train(student, train_loader, test_loader, unlabelled_train_loader, args):
    ...
    cluster_criterion = DistillLoss4_var(
    ...
    for epoch in range(args.epochs):
        ...
        #         feats = student.encoder_q[0](images)  
        ...
        #     centers_classifier = torch.tensor(kmeans.cluster_centers_).cuda()
        #     student.encoder_q[1].last_layer.weight=centers_classifier
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                contrastive_logits, contrastive_labels, tailness, student_out, student_feat, sup_logits, targets = student(images[0].cuda(non_blocking=True),images[1].cuda(non_blocking=True),labels_for_unimoco, args)
                ...
                # student_proj = torch.cat([f[mask_lab].unsqueeze(1) for f in student_feat.chunk(2)], dim=1)
                # student_proj = torch.nn.functional.normalize(student_proj, dim=-1)
                # sup_tailness = tailness[mask_lab]
        ...
        for i,j in enumerate(var_epoch):
                if j == 0 :
                    # var[i] = var[i] + 0.1 * torch.max(var_epoch)
                    var[i] = 1
                    # var[i] = var[i]
                else:
        ...
        # if (epoch+1) % 10 ==0:
        #     model.variance_pseudo_epoch0 = torch.tensor(var).cuda()
        #     var_pseudo_known = [var[i] for i in known_descending]
        #     var_pseudo_unknown = [var[i] for i in unknown_descending]
        #     np.save(f'/data2/kh12043/bacon_new/vars/var_pseudo_known_{args.rev}_{epoch+1}', np.array(var_pseudo_known))
        #     np.save(f'/data2/kh12043/bacon_new/vars/var_pseudo_unknown_{args.rev}_{epoch+1}', np.array(var_pseudo_unknown))
        #     plt.plot(x_values_known,var_pseudo_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_pseudo_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig(f'/data2/kh12043/bacon_new/vars/figs/variance_std_max_ema_30init_mask_unimoco3_{args.rev}_pseudo_epoch{epoch+1}.png')
        #     plt.clf()
            
            
        #     model.variance_gt_epoch0 = torch.tensor(var_gt).cuda()
        #     var_gt_known = [var_gt[i] for i in known_descending]
        #     var_gt_unknown = [var_gt[i] for i in unknown_descending]
        #     np.save(f'/data2/kh12043/bacon_new/vars/var_gt_known_{args.rev}_{epoch+1}', np.array(var_gt_known))
        #     np.save(f'/data2/kh12043/bacon_new/vars/var_gt_unknown_{args.rev}_{epoch+1}', np.array(var_gt_unknown))
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig(f'/data2/kh12043/bacon_new/vars/figs/variance_std_max_ema_30init_mask_unimoco3__{args.rev}_gt_epoch{epoch+1}.png')
        #     plt.clf()
            
            # model.tailness_epoch0_update = torch.tensor(tailness_gt_epoch).cuda()
            # tail_gt_known = [tailness_gt_epoch[i] for i in known_descending]
            # tail_gt_unknown = [tailness_gt_epoch[i] for i in unknown_descending]
            # plt.plot(x_values_known,tail_gt_known,label='Density_Known')
            # plt.plot(x_values_unknown,tail_gt_unknown,label='Density_UnKnown')
            # plt.legend()
            # plt.savefig('tailness_std_max_ema_30init_mask_unimoco3_epoch0_update.png')
            # plt.clf()
            
          
        # if epoch == 29:
        #     model.variance_pseudo_epoch30 = torch.tensor(var).cuda()
        #     var_gt_known = [var[i] for i in known_descending]
        #     var_gt_unknown = [var[i] for i in unknown_descending]
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig('variance_std_max_ema_30init_mask_unimoco3_pseudo_epoch30.png')
        #     plt.clf()
            
            
        #     model.variance_gt_epoch30 = torch.tensor(var_gt).cuda()
        #     var_gt_known = [var_gt[i] for i in known_descending]
        #     var_gt_unknown = [var_gt[i] for i in unknown_descending]
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig('variance_std_max_ema_30init_mask_unimoco3_gt_epoch30.png')
        #     plt.clf()
            
        #     model.tailness_epoch30_update = torch.tensor(tailness_gt_epoch).cuda()
        #     tail_gt_known = [tailness_gt_epoch[i] for i in known_descending]
        #     tail_gt_unknown = [tailness_gt_epoch[i] for i in unknown_descending]
        #     plt.plot(x_values_known,tail_gt_known,label='Density_Known')
        #     plt.plot(x_values_unknown,tail_gt_unknown,label='Density_Known')
        #     plt.legend()
        #     plt.savefig('tailness_std_max_ema_30init_mask_unimoco3_epoch30_update.png')
        #     plt.clf()
        
        # if epoch == 99:
        #     model.variance_pseudo_epoch100 = torch.tensor(var).cuda()
        #     var_gt_known = [var[i] for i in known_descending]
        #     var_gt_unknown = [var[i] for i in unknown_descending]
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig('variance_std_max_ema_30init_mask_unimoco3_pseudo_epoch100.png')
        #     plt.clf()
            
            
        #     model.variance_gt_epoch100 = torch.tensor(var_gt).cuda()
        #     var_gt_known = [var_gt[i] for i in known_descending]
        #     var_gt_unknown = [var_gt[i] for i in unknown_descending]
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig('variance_std_max_ema_30init_mask_unimoco3_gt_epoch100.png')
        #     plt.clf()
            
        #     model.tailness_epoch100_update = torch.tensor(tailness_gt_epoch).cuda()
        #     tail_gt_known = [tailness_gt_epoch[i] for i in known_descending]
        #     tail_gt_unknown = [tailness_gt_epoch[i] for i in unknown_descending]
        #     plt.plot(x_values_known,tail_gt_known,label='Density_Known')
        #     plt.plot(x_values_unknown,tail_gt_unknown,label='Density_Known')
        #     plt.legend()
        #     plt.savefig('tailness_std_max_ema_30init_mask_unimoco3_epoch100_update.png')
        #     plt.clf()
            
        # if epoch == 199:
        #     model.variance_pseudo_epoch200 = torch.tensor(var).cuda()
        #     var_gt_known = [var[i] for i in known_descending]
        #     var_gt_unknown = [var[i] for i in unknown_descending]
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig('variance_std_max_ema_30init_mask_unimoco3_pseudo_epoch200.png')
        #     plt.clf()
            
            
        #     model.variance_gt_epoch200 = torch.tensor(var_gt).cuda()
        #     var_gt_known = [var_gt[i] for i in known_descending]
        #     var_gt_unknown = [var_gt[i] for i in unknown_descending]
        #     plt.plot(x_values_known,var_gt_known,label='Std_Known')
        #     plt.plot(x_values_unknown,var_gt_unknown,label='Std_Unknown')
        #     plt.legend()
        #     plt.savefig('variance_std_max_ema_30init_mask_unimoco3_gt_epoch200.png')
        #     plt.clf()
            
        #     model.tailness_epoch200_update = torch.tensor(tailness_gt_epoch).cuda()
        #     tail_gt_known = [tailness_gt_epoch[i] for i in known_descending]
        #     tail_gt_unknown = [tailness_gt_epoch[i] for i in unknown_descending]
        #     plt.plot(x_values_known,tail_gt_known,label='Density_Known')
        #     plt.plot(x_values_unknown,tail_gt_unknown,label='Density_Known')
        #     plt.legend()
        #     plt.savefig('tailness_std_max_ema_30init_mask_unimoco3_epoch200_update.png')
        #     plt.clf()
...
if __name__ == "__main__":
    ...
    init_experiment(args, runner_name=['Ours'])
    ...
    all_feats = np.concatenate(all_feats)
    kmeans = KMeans(n_clusters=100, random_state=0).fit(all_feats)
