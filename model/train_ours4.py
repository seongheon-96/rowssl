...
from model.moco_cifar_k25 import MoCo
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                # print('ind', ind.shape)
                # print('tailness', tailness.shape)
                # print('queue_ps', queue_pseudo.shape)
...
def test_inductive(model, test_loader, epoch, save_name, ind, args):
    ...
    # total_pred_all_instances = 0
    # for i in all_classes_gt:
    #     all_acc += w[ind[i], i]
    #     total_all_instances += sum(w[:, i])
    #     total_pred_all_instances += sum(w[ind[i], :])

    # if total_all_instances != 0:
    #     all_acc /= total_all_instances
    #     all_acc *= 100
    # else:
    #     all_acc = -1.
    ...
    #     total_pred_old_instances += sum(w[ind[i], :])
    ...
    #     total_pred_new_instances += sum(w[ind[i], :])
...
if __name__ == "__main__":
    ...
    args.mlp_out_dim = 200#args.num_labeled_classes + args.num_unlabeled_classes
