...
from model.model_ours import DINOHead_org, info_nce_logits, SupConLoss, DistillLoss, ContrastiveLearningViewGenerator, get_params_groups
from model.moco_base_rep import MoCo
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                contrastive_logits, contrastive_labels, tailness, student_out, student_feat, sup_logits, target, cluster, cluster_center, queue, label_queue = student(images[0].cuda(non_blocking=True),images[1].cuda(non_blocking=True),labels_for_unimoco,class_labels, args)
                ...
                # clustering, unsup
                cluster_loss = cluster_criterion(student_out, teacher_out,None,None, epoch)
                ...
                # student_proj = torch.cat([f[mask_lab].unsqueeze(1) for f in student_out.chunk(2)], dim=1)
        ...
        if epoch in [1,99,199]:
            torch.save(torch.cat(ls), f'check/cifar100/labels_ep{epoch}.pth')
            torch.save(torch.cat(dens), f'check/cifar100/dens_ep{epoch}.pth')
            torch.save(torch.cat(clus), f'check/cifar100/clus_idx_ep{epoch}.pth')
            torch.save(cluster_center, f'check/cifar100/cl_center_ep{epoch}.pth')
            torch.save(queue, f'check/cifar100/queue_ep{epoch}.pth')
            torch.save(label_queue, f'check/cifar100/label_queue_ep{epoch}.pth')
...
def test_bacon(model, test_loader, epoch, save_name, args):
    ...
    for batch_idx, (images, label, _) in enumerate(tqdm(test_loader)):
        # images = images.cuda(non_blocking=True)
        with torch.no_grad():
            feats, _, _ = model(images.cuda(non_blocking=True))
...
if __name__ == "__main__":
    ...
    # PROJECTION HEAD
    # ----------------------
    # projector = DINOHead_org(in_dim=args.feat_dim, out_dim=args.mlp_out_dim, nlayers=args.num_mlp_layers)
    # model = nn.Sequential(backbone, projector).to(device)
    proj_moco = MoCo(base_encoder=backbone, ema_encoder=ema_backbone, args=args, centers_classifier = None)
    model = proj_moco.to(device)
    
    # checkpoint = torch.load("/media/diml-03/shpark_gcd/shpark/GCD/simgcd/dev_outputs/Experiments_Memory/cifar100_simgcd_Orginal_(09.06.2023_|_46.402)/checkpoints/model_best.pt")
