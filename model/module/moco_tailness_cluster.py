...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k, args, train=True):
        """
        Input:
        ...
        density_estimation = torch.zeros(similarity_matrix.shape[0]).detach()
        
        for i in range(similarity_matrix.shape[0]):
            density_estimation[i] =  (1/5) * torch.sort(similarity_matrix[i],descending=True)[0][:5].sum().clone().detach() +  (1/10) * torch.sort(similarity_matrix[i],descending=True)[0][:10].sum().clone().detach() +  (1/15) * torch.sort(similarity_matrix[i],descending=True)[0][:15].sum().clone().detach()
            density_estimation[i] = (1/3) * density_estimation[i]
        ...
        return logits, labels, density_estimation[instance_assign] , torch.cat((q_out,q_out2),dim=0), torch.cat((q_proj,q_proj2))
