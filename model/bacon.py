...
def train_dual(ce_backbone, ce_head, cl_backbone, cl_head, train_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.test_freq == 0:
            ...
            with torch.no_grad():
                all_acc_test_cl_t, old_acc_test_cl_t, new_acc_test_cl_t, acc_list_cl_t, cl_ind_map_t, kmeans = test_transductive(
                    student_cl,
            ...
            args.logger.info(
                    'Transductive Test Accuracies CL: All {:.1f} | Old {:.1f} | New {:.1f}'.format(all_acc_test_cl_t,
                                                                                        ...
                                                                                        new_acc_test_cl_t))
            # with torch.no_grad():
            #     all_acc_test_cl_t, old_acc_test_cl_t, new_acc_test_cl_t, acc_list_cl_t, cl_ind_map_t, kmeans = test_kmeans_semi(
            #         student_cl,
            #         test_loader=train_loader,
            #         epoch=epoch,
            #         save_name='Transductive Test ACC',
            ...
            with torch.no_grad():
                all_acc_test_cl, old_acc_test_cl, new_acc_test_cl, acc_list_cl, cl_ind_map = test_inductive(
                    student_cl,
                    test_loader,
                    kmeans,
                    epoch=epoch,
...
def test_transductive(model, test_loader, epoch, save_name, args, train_loader):
    ...
    kmeans = KMeans(n_clusters=K, init_method='random', verbose=1)
    preds = kmeans.fit_predict(all_feats).cpu().numpy()
    print('Done!')
    ...
    return all_acc, old_acc, new_acc, acc_list, ind_map, kmeans
...
def test_kmeans_semi(model, test_loader, epoch, save_name, args, train_loader):
    ...
    all_acc, old_acc, new_acc, acc_list, ind_map = log_accs_from_preds(y_true=u_targets, y_pred=preds, mask=mask,
                                                    T=epoch, eval_funcs=args.eval_funcs, save_name=save_name,
                                                    args=args, train_loader=train_loader)
...
if __name__ == "__main__":
    ...
    if args.dataset_name == 'inaturelist18':
        total_class = 1000
    print(total_class)
