...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, ema_encoder, args, centers_classifier, dim=256, K=8192, m=0.999, T=0.07, mlp=True):
        """
        ...
        self.register_buffer("label_queue2", torch.zeros(self.K).long())
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys, labels, labels_cheat):
        ...
        self.label_queue2[ptr:ptr + batch_size] = labels_cheat
    ...
    def forward(self, im_q, im_k, class_labels, labels_cheat, args, train=True):
        """
        Input:
        ...
        # negative logits: NxK
        # l_neg = torch.einsum("nc,ck->nk", [q_proj, self.queue.clone().detach()]) # 128,K
        l_neg = torch.einsum("nc,ck->nk", [q_proj, self.queue.clone().detach()]) # 128,K
        ...
        instance_assign = torch.argmax(assign_matrix,dim=-1) # 여기서 argmax 대신 max 쓰면 query와 가장 가까운 cluster 간 거리?
        dist=torch.max(assign_matrix,dim=-1)[0]
        similarity_matrix = torch.matmul(self.cluster_center, self.queue.clone().detach()) #100,K
        ...
        # # 텐서를 0과 1 사이로 정규화
        # dist_normalized = (dist - min_val) / (max_val - min_val)

        # #
        # dist_scaled = dist_normalized * (args.max_tau - args.min_tau) + args.min_tau
        
        instance_wise_tau = tau[instance_assign].cuda().detach()
        
        # instance_wise_tau = dist_scaled#0.07*torch.ones_like(instance_wise_tau_) # no dynamic temperature
        ...
        lq_ = self.label_queue2.clone().detach()
        
        self._dequeue_and_enqueue(k_proj, class_labels, labels_cheat)
        ...
        return logits, labels, density_estimation[instance_assign] , torch.cat((q_out,q_out2),dim=0), torch.cat((q_proj,q_proj2)), sup_logits, targets, instance_assign, self.cluster_center, queue_, lq_
