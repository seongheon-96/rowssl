...
from util.cluster_and_log_utils_herb import log_accs_from_preds
    ...
    for epoch in range(args.epochs):
        ...
        args.logger.info(
                    'Transductive Test Accuracies CL: All {:.1f} | Old {:.1f} | New {:.1f}'.format(all_acc_test_cl_t,
                                                                                        old_acc_test_cl_t,
                                                                                        new_acc_test_cl_t))
        # args.logger.info('Testing like bacon...')
            
        # with torch.no_grad():
        #         all_acc_test_cl_tt, old_acc_test_cl_tt, new_acc_test_cl_tt, acc_list_cl_tt, bacc_list_cl_tt, _ = test_bacon(
        #             model,
        ...
        #             train_loader=train_loader)
        # args.logger.info(
        #             'Transductive Test Accuracies CL: All {:.1f} | Old {:.1f} | New {:.1f}'.format(all_acc_test_cl_tt,
                                                                                        # old_acc_test_cl_tt,
                                                                                        # new_acc_test_cl_tt))
...
def test_inductive(model, test_loader, kmeans_train, epoch, save_name, args, train_loader):
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        # mask = np.array([True if x.item() in args.train_classes else False for x in targets])
...
def test_inductive_wom(model, test_loader,kmeans_train, epoch, save_name, ind, args,train_loader):
...
if __name__ == "__main__":
    ...
    init_experiment(args, runner_name=['Kmeans'])
