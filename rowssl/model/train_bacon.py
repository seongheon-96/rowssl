...
from util.cluster_and_log_utils_cub import log_accs_from_preds
from config import exp_root
from bacon_module.loss import info_nce_logits, SupConLoss, DistillLoss, ContrastiveLearningViewGenerator, get_params_groups
from copy import deepcopy
# from sklearn.cluster import KMeans
# from cuml.cluster import KMeans as cuKMeans
from data.cifar import CustomCIFAR100, cifar_100_root
import random
from bacon_module.dist_est import dist_est
from bacon_module.reg_loss import compute_reg_loss
from bacon_module.softconloss import compute_softconloss
import os
import sys
...
def train_dual(ce_backbone, ce_head, cl_backbone, cl_head, train_loader, test_loader, args):
    ...
    from util.cluster_and_log_utils_cub import set_args_mmf
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.test_freq == 0:
            ...
            with torch.no_grad():
                all_acc_test_cl_b, old_acc_test_cl_b, new_acc_test_cl_b, acc_list_cl_b, bacc_list_cl_b, cl_ind_map_b = test_bacon(
                    student_cl,
                    test_loader,
                    epoch=epoch,
                    save_name='Test bacon ACC',
                    args=args,
                    train_loader=train_loader)
                
            args.logger.info('Testing wo matching on disjoint test set...')
            with torch.no_grad():
                all_acc_test_cl2, old_acc_test_cl2, new_acc_test_cl2, manyo, medo, fewo, manyn, medn, fewn = test_inductive_wom(
                    student_cl,
                    test_loader,
                    kmeans,
                    epoch=epoch,
...
def test_inductive_wom(model, test_loader,kmeans_train, epoch, save_name, ind, args):
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        # all_feats.append(feats.detach().cpu().numpy())
        targets = np.append(targets, label.cpu().numpy())
        if args.dataset_name in['cub','scars','aircraft']:
                mask = np.append(mask, np.array([True if x.item() in args.train_classes #for cub
                                            else False for x in label]))
        # else:
        #     mask = np.append(mask, np.array([True if x.item() in range(len(args.train_classes))
    ...
    # total_acc = sum([w[i, j] for i, j in ind]) * 1.0 / preds.size
    # total_acc *= 100
...
def test_transductive(model, test_loader, epoch, save_name, args, train_loader):
    ...
    kmeans = KMeans(n_clusters=args.num_labeled_classes + args.num_unlabeled_classes, verbose=1)
...
def test_bacon(model, test_loader, epoch, save_name, args, train_loader):
    model.eval()
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        all_feats.append(feats.detach().cpu().numpy())
        targets = np.append(targets, label.cpu().numpy())
        # mask = np.append(mask, np.array([True if x.item() in range(len(args.train_classes)) #inat
        #                                  else False for x in label]))
        mask = np.append(mask, np.array([True if x.item() in args.train_classes #for cub
                                         else False for x in label]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rev', default='consis', type=str)
    ...
    print('zz',args.num_labeled_classes, args.num_unlabeled_classes)
    ...
    from bacon_module import vision_transformer as vits
