...
def train_split(labels, n_labeled_per_class, n_unlabeled_per_class, train_classes): 
    ...
    unlabeled_classes = list(set(np.arange(100)) - set(train_classes))
    for i in range(100):
        ...
        if i in train_classes:
            ...
            train_unlabeled_idxs.extend(idxs[:n_unlabeled_per_class[50 + torch.where((torch.tensor(unlabeled_classes))==i)[0].item()]])
...
def get_aircraft_datasets(train_transform, test_transform, train_classes=range(50), prop_train_labels=0.8,
    ...
    if rev == 'consis':
            ...
            u_samples_unknown = make_imb_data(30, 100 - len(train_classes) , imb, 0, rev)
            ...
            u_samples_known[-1] -= 1
            u_samples_known[-2] -= 1
            u_samples_unknown = make_imb_data(30, 100 - len(train_classes) , imb, 0, 1)
            u_samples = u_samples_known + u_samples_unknown 
    ...
    train_labeled_idxs, train_unlabeled_idxs = train_split([i[1] for i in whole_training_set.samples], l_samples, u_samples, train_classes)
    ...
    unlabelled_classes = list(set(whole_training_set.class_to_idx.values()) - set(train_classes))
