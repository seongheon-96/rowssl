...
class cifar_imb(Dataset):
    def __init__(self, imbanlance_rate=0.1, file_path="data/cifar-100-python/", num_cls=100, transform=None,
        ...
        self.uq_idxs = np.array(range(len(self.x)))
        self.data = self.x
    ...
    def __getitem__(self, item):
        ...
        x, y = self.data[item], self.targets[item]
        x = Image.fromarray(x)
    ...
    def produce_imbanlance_data(self, imbanlance_rate):
        ...
        for cls_idx in range(self.num_cls):
            num = data_num * (imbanlance_rate ** (cls_idx / (self.num_cls - 1))) # divide
