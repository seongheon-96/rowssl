...
from config import place_root
import torch

class PlacesBase(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform):

        super(PlacesBase, self).__init__(root, transform)
...
def get_places_LT_datasets(train_transform, test_transform, train_classes=range(182),
                           prop_train_labels=0.5, split_train_val=False, seed=0, args=None, imb=1, rev='consis'):

    from data.LT_dataset import LT_Dataset
    txt_path = "/data2/kh12043/datasets/Places/Places_LT_train.txt"
    test_txt_path = "/data2/kh12043/datasets/Places/Places_LT_test.txt"
    total_class = 365
    anno_ratio = 50
    seed = 416
    ...
    whole_training_set = LT_Dataset(root=place_root, txt=txt_path, transform=train_transform)
    ...
    test_dataset = LT_Dataset(root=place_root, txt=test_txt_path, transform=test_transform)
